...
//    makeJasoByPoint();
    ...
    for(var i = 0; i < letterArray.length; i++){
        var rows = letterArray[i];
        var m = new Mover();
        moverArray.push(m);
    }
    console.log("moverArray.length : " + moverArray.length);
...
//        console.log(moverArray[i]);
        moverArray[i].update(letterArray[i].x, letterArray[i].y);
        ...
        function 리을(){
           ...
           //좌표데이터로 객체 생성
            for(var i =0; i < data.length; i++){
                ...
                var from = {x:row.x1, y:row.y1};
                var to = {x:row.x2, y:row.y2};
                var eachPos = (1.0/ops.ball_len);
                
                resultArray.push(from);

                for(var j=0; j < ops.ball_len; j++){
                    var currentX = lerp(from.x, to.x, eachPos);
                    var currentY = lerp(from.y, to.y, eachPos);

                    var pos = {x:currentX, y:currentY};
                    resultArray.push(pos);
                    eachPos += 1.0/ops.ball_len;
                }
                resultArray.push(to);
            }   
            
            //생성한 좌표배열로 객체를 생성하고 바로 객체배열에 집어넣는다.
            for(var index=0; index<resultArray.length; index++){
                letterArray.push(resultArray[i]);
            }
        }
