//for runner
console.log("runner_attractor.0.2.js");
var moverNum = 100;
var moverArray =[];
var mover;
...
function makeJasoByPoint(){
    ...
    var pointNum = 4; //시작점과 끝점 사이 점 개수
    var gap = {jaso:100};
    //기억
...
//    test
//    makePointByLerp(10, 10, 120, 10, 10);
...
//        strokeWeight(2);
        ...
        fill(255);
        ellipse(this.location.x, this.location.y, ops.ballSize, ops.ballSize);
    }
...
class Attractor{
    ...
    constructor(targetX, targetY){
        this.mass = 20;
        this.location = createVector(targetX, targetY);
        this.g = 0.4;
    ...
    attract(movers){
        this.force = p5.Vector.sub(this.location, movers.location);
        var distance = this.force.mag();
        
        //중력이 너무 자근 값 또는 큰 값을 갖지 않게 보정
        this.distance = constrain(this.distance, 5.0, 25.0);
        
        this.force.normalize();
        var strength = (this.g * this.mass * movers.mass) / (this.distance * this.distance);
...
function mousePressed(){
    console.log("noLoop by mousePressed")
    noLoop();
    background(0, 230);
}